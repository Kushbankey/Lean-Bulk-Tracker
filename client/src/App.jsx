...
import { api } from "./api";
import { PHASE_CONFIG, getPhaseForWeek, getWeekNumber, getTodayKey, QUOTES } from "./data";
...
export default function App() {
  ...
  const ctx = { settings, logs, stats, week, phase, todayQuote, latestWeight, gained, remaining, progress, calorieTarget, fetchAll };

  return (
    <div style={{ minHeight: "100vh", paddingBottom: 80 }}>
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #0f1923 0%, #0a1628 100%)", padding: "24px 20px 20px", borderBottom: "1px solid #1a2535" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: -1 }}>
              ...
              <span style={{ fontSize: 13, color: "#4a6080", fontWeight: 400 }}> of 32</span>
            </div>
          </div>
      ...
      <div style={{ padding: 16 }} className="fade-in">
        ...
        {tab === "plan" && <Roadmap ctx={ctx} />}
      </div>

      {/* Bottom Tab Bar */}
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#0a0e14", borderTop: "1px solid #1a2535", display: "flex", zIndex: 50, paddingBottom: "env(safe-area-inset-bottom)" }}>
        {TABS.map((t) => (
      ...
      {/* Quick Log FAB */}
      {tab !== "log" && (
        <button onClick={() => setTab("log")} style={{ position: "fixed", bottom: 80, right: "calc(50% - 210px)", width: 52, height: 52, borderRadius: "50%", background: phase.color, border: "none", color: "#080c10", fontSize: 24, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 20px ${phase.color}44`, zIndex: 40, display: "flex", alignItems: "center", justifyContent: "center", transition: "transform 0.2s" }}
